    ...
    <form action="{{ route('user.update', $user->id) }}" method="post">
        ...
        <select name="region" id="region">
            ...
            @foreach ($regions as $region)
                <option value="{{ $region['nom'] }}" {{ $user->region == $region['nom'] ? 'selected' : '' }}>{{ $region['nom'] }}</option>
            @endforeach
        </select>

        <input type="text" id="city" name="city" placeholder="Ville" value="{{ $user->city }}">

        <input type="text" id="postal_code" name="postal_code" placeholder="Code postal" value="{{ $user->postal_code }}">
